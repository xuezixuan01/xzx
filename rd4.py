...
hzlb=float(sys.argv[1])
hzgb=float(sys.argv[2])
type=sys.argv[3]
# hzlb=-0.5
# hzgb=2
# type="TMS"
hz_total = proton.basicPars["status SW_p"]
ppm_total=float(proton.getPar('SW'))
right_edge=float(proton.getPar('F1P'))
ppm_center=float(proton.getPar('O1P'))
ppmlow = proton.basicPars["status OFFSET"]
left_shift=float(proton.getPar('F1P'))
right_shift=float(proton.getPar('F2P'))
center_shift=float(proton.getPar('OFFSET'))


orgspec=t['dataPoints']
total_point=len(orgspec)
resarea=t['dataPoints'].copy()
intRegions = proton.getIntegrationRegions()
shift_per_point=total_point/ppm_total
left_point=int((ppmlow-left_shift)*shift_per_point)
right_point=int((ppmlow-right_shift)*shift_per_point)
center_point=int(ppmlow*shift_per_point)
...
for p in range(0,len(prexx)):
    if abs(p-center_point)>0.1:
        ...
        prexx[p]=100000
###################
prexx=reference_type(prexx,type,len(prexx)/ppm_total,center_point,100000,ppm_total/hz_total,sf)
